...
<div class="row justify-content-center">
  <div class="col-lg-10 col-md-12">
    <div class="card shadow-lg" style="border:3px solid #ffb347;background:linear-gradient(135deg,#fffde7 0%,#e1f5fe 100%);">
      <div class="card-body">
        <h3 class="mb-4 text-center" style="color:#ff9800;"><i class="bi bi-cart4"></i> Carrito de compras</h3>
        <?php if (empty($items)): ?>
          ...
          <table class="table align-middle" style="background:rgba(255,255,255,0.95);border-radius:12px;overflow:hidden;">
            ...
            <tbody>
              ...
              <tr data-id="<?=$item['id']?>" style="vertical-align:middle;">
                <td>
                  <div class="d-flex align-items-center">
                    <?php if (!empty($item['image'])): ?>
                      <img src="<?=htmlspecialchars($item['image'])?>" alt="" style="width:60px;height:60px;object-fit:cover;border-radius:12px;margin-right:12px;border:2px solid #b2dfdb;">
                    <?php else: ?>
                ...
                <td style="color:#ff9800;font-weight:500;">$<?=number_format($item['price'],2)?></td>
                <td>
                  <button class="btn btn-sm btn-outline-warning btn-cart-dec" data-id="<?=$item['id']?>"><i class="bi bi-dash"></i></button>
                  <span class="cart-qty mx-2" style="font-size:1.1em;"><?=$item['quantity']?></span>
                  <button class="btn btn-sm btn-outline-success btn-cart-inc" data-id="<?=$item['id']?>"><i class="bi bi-plus"></i></button>
                </td>
                <td style="color:#388e3c;font-weight:500;">$<?=number_format($item['price'] * $item['quantity'],2)?></td>
                <td>
        ...
        <div class="d-flex justify-content-between mt-4">
          <a href="index.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> Volver al catálogo</a>
          <a href="index.php?page=checkout" class="btn btn-primary" style="background:#388e3c;"><i class="bi bi-credit-card"></i> Proceder a pago</a>
        </div>
        <?php endif; ?>
      </div>
    </div>
